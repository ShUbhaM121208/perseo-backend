...
router.get('/list-connections', async (req, res) => {
  try {
    ...
    console.log('All connected accounts:', connections.items.map((conn: any) => ({
      id: conn.id,
      status: conn.status,
  ...
  try {
    ...
    try {
      ...
      const activeConnection = connections.items.find((conn: any) => 
        conn.id === connectionId && conn.status === 'ACTIVE'
      );
...
router.post('/send', async (req, res) => {
  try {
    ...
    const connections = await composio.connectedAccounts.list();
    const activeConnection = connections.items.find((conn: any) => 
      conn.id === connectedAccountId && conn.status === 'ACTIVE'
    );
    ...
    let entityId: string;
    try {
      const connectionData = (activeConnection as any).data;
      if (!connectionData || !connectionData.id_token) {
        throw new Error('Token data not found');
      }
      
      const idToken = connectionData.id_token as string;
...
router.get('/message/:messageId', async (req, res) => {
  try {
    ...
    const extractedData: any = messageData;
    ...
    return res.json({
      ...
      data: {
        ...
        attachments: extractedData.attachments || extractedData.payload?.parts?.filter((part: any) => part.filename) || [],
...
router.get('/tools', async (req, res) => {
  try {
    ...
    console.log('\n=== SEND/COMPOSE Tools ===');
    sendTools.forEach((tool: any) => console.log(`- ${tool.slug}`));
    
    console.log('\n=== GET/LIST Tools ===');
    getTools.forEach((tool: any) => console.log(`- ${tool.slug}`));
    
    return res.json({
      success: true,
      totalTools: tools.length,
      sendTools: sendTools.map((t: any) => t.slug),
      getTools: getTools.map((t: any) => t.slug),
...
router.get('/debug-connection', async (req, res) => {
  try {
    ...
    const connections = await composio.connectedAccounts.list();
    const targetConnection = connections.items.find((conn: any) => conn.id === connectedAccountId);
    ...
    try {
      const connectionData = (targetConnection as any).data;
      if (connectionData && connectionData.id_token) {
        const idToken = connectionData.id_token as string;
        const tokenParts = idToken.split('.');
        const payload = JSON.parse(Buffer.from(tokenParts[1], 'base64').toString());
...
router.get('/available-tools', async (req, res) => {
  try {
    ...
    const userIdToUse = (userId as string) || 'shubhamm.0010@gmail.com';
